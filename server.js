...
app.post("/api/reply", (req, res) => {
    ...
    db.query("select * from messages where customer_phone = ?", [from], (err, result) => {
        ...
        db.query("insert into messages (xero_invoice_id, heading, para, type, date, customer_phone) values (?, ?, ?, ?, ?, ?)", [xeroId, "Customer Response", body, "response", getCurrentDate(), from], (err, result) => {
            ...
            db.query("select * from invoices where xero_id = ?", [xeroId], async (err, result) => {
                if(err){
                    console.error(err);
                }
                ...
                if(result[0] && result[0].cancelled == "no"){
                    ...
                    */
                    ...
                    db.query("insert into messages (xero_invoice_id, heading, para, type, date, customer_phone) values (?, ?, ?, ?, ?, ?)", [xeroId, "AI Response", aiResponse, "airesponse", getCurrentDate(), from], (err, result) => {
                        if(err){
                            console.error(err);
                        }
